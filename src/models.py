...
class TranCIRCE(nn.Module):
	def __init__(self, feats):
		super(TranCIRCE, self).__init__()
		self.name = 'TranCIRCE'
		self.lr = lr
		self.batch = 128
		self.n_feats = feats
	...
	def forward(self, src, tgt, label = 0):
		# Phase 1 - Without anomaly scores
		c = torch.zeros_like(src)
		x1 = self.fcn(self.transformer_decoder1(*self.encode(src, c, tgt)))
...
class OSContrastiveTransformer(nn.Module):
	def __init__(self, feats):
		super(OSContrastiveTransformer, self).__init__()
		self.name = 'OSContrastiveTransformer'
		self.lr = lr
		self.batch = 128
		self.n_feats = feats
		self.n_window = 10
